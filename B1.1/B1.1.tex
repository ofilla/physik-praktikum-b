...
Dazu werden eine Gaußverteilung der Breite $\Delta \nu=1\mathrm{\,Hz}$ angenommen. Der Grundabsorptionskoeffizient $\alpha_0$ wird als Fitparameter $px$ dargestellt, weiterhin wird der Bandenursprung auf die Frequenz $\nu_0=0\mathrm{\,Hz}$ gesetzt.

\begin{eqnarray}
	T(C) &=& \frac{I(C)}{I_0} \\
		&=& \int_{\nu_1}^{\nu_2} \exp[- px \cdot C \cdot e^{- \nu^{\prime\,2}}] d\nu^\prime
\end{eqnarray}
...
\noindent
Durch die Wahl des Bandenursprungs $\nu_0=0\mathrm{\,Hz}$ ist das zu betrachtende Intervall beliebig, so lange es symmetrisch um $0\mathrm{\,Hz}$ liegt und fast die ganze Fläche der in der Summe auftauchenden Exponentialfunktion betrachtet. Um die Näherung zu rechtfertigen, muss die Schrittweite $s_\nu$ klein sein.

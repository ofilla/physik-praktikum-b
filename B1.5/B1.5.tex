...
Das magnetische \emph{Dipolmoment} $\vec \mu$ tritt auf, wenn sich elektrische Ladungen bewegen. Es lässt sich über das auf einen magnetischen Dipol wirkende Drehmoment $\vec \tau$ in einem Magnetfeld $\vec B$ definieren. Für eine ebene Leiterschleife ist es durch Gleichung \eqref{eq:Dipolmoment} beschrieben. \cite{Jackson}

Damit ist das Dipolmoment $\vec \mu$ parallel zum Drehimpuls $\vec{L}$. Die Energie $E$ zur Ausrichtung eines Dipolmomentes wird durch das Skalarprodukt $\vec \mu \cdot \vec B$ beschrieben.

\begin{eqnarray}
	\vec \tau &=& \vec \mu \times \vec B \label{eq:Dipolmoment}\\
...
Der \emph{normale Zeemann--Effekt} tritt auf, wenn der Spin der betrachteten Teilchen gleich $0$ ist. Beim \emph{anormalen Zeemann--Effekt} hingegen ist der Spin der betrachteten Teilchen ungleich $0$. In diesem Versuch ist der anormale Zeemann--Effekt relevant, da der Spin der Elektronen nicht verschwindet.
...
Aufgrund des Zeemann--Effekts kommt es zur Lamorpräzession der Elektronen, bei der der Spin rotiert. Das magnetische Moment $\mu$ eines Spins weist im Magnetfeld $\vec{B}$ durch Quantenschwebung eine Präzession mit einer \emph{Lamorfrequenz} $\omega_L$ auf.
...
Erst in Anwesenheit eines äußeren Magnetfeldes richten sich die Dipolmomente entlang des Feldes aus. Daher ist die magnetische Suszeptibilität $\chi$ eines Paramagneten positiv. Diese beschreibt die Änderung der Magnetisierung $M$ durch die Änderung der magnetischen Feldstärke $H$ \eqref{eq:suszeptibilität}.
...
\begin{eqnarray}
	\chi &=& \frac{\mathrm dM}{\mathrm dH} \label{eq:suszeptibilität} \\
	\chi &=& \frac{C}{T}
\end{eqnarray}
...
Die Impedanz einer Spule $Z_L$ wird durch die Induktivität $L$ bestimmt, die Impedanz eines Kondensators $Z_C$ durch die Kapazität $C$.
...
Dabei werden zwei Wechselstromwiderstände, bestehend aus einem Ohm'schen Widerstand $R$ und einem Kondensator $C$, in Reihe geschaltet. Parallel dazu wird ein Widerstand geschaltet, der mit einem Schleifdraht geregelt werden kann. Dabei sind $R_1$ der zu bestimmende Widerstand in der Masche.
...
Durch das Oszilloskopsignal wird der Abgriff des Schleifdrahtes in die Mitte gebracht. Der Nullabgleich erfolgt dadurch, dass das Signal am Oszilloskop durch eine geeignete Wahl  von $R_2$ auf ein Minimum gebracht wird. Der Feinabgleich erfolgt mit dem Schleifdraht; alternativ kann $R_3$ ein regelbarer Widerstand sein.
...
Die Absorption von Strahlungsquanten beginnt dann in der Probe, wenn das Magnetfeld der Helmholtzspulen die Resonanzflussdichte $B_r$ erreicht. Damit ändert sich die Impedanz des Schwingkreises.
...
Es wird ein Strom zwischen $1.0\mathrm{\, A}$ und $1.5 \mathrm{\, A}$ eingestellt. Die Kapazität des Resonators wird so konfiguriert, dass zwei möglichst symmetrische Glockenkurven auf dem Oszilloskop sichtbar sind. Dann wird die Phase so eingestellt, dass sich beide Kurven möglichst gut überlagern.
...
Zur Bestimmung des g-Faktors von DPPH aus der gemessenen Resonanzstromstärke $I_r$ werden zunächst einige Überlegungen angestellt.
...
Dabei wird das entsprechende Resonanzmagnetfeld $B_r$ gemessen. Der Landé--Faktor $g$ folgt durch Gleichsetzen der Gleichung \eqref{eq:deltaE} mit der Energie der Mikrowellenstrahlung \eqref{eq:Energie Strahlung}.
...
\noindent
Der theoretische Wert beträgt $2.7 \cdot 10^{-4} \mathrm{\, T}$. \cite{Uni} Das vorliegende Ergebnis hat somit die selbe Größenordnung, stimmt mit dem theoretischen Wert aber auch innerhalb der Fehlergrenzen nicht überein.
...
Analog zu Abschnitt \ref{durchfuxfchrung:Halbwertsbreite} können die Resonanzstromstärken $\bar{I}_r$ und die entsprechenden magnetischen Flussdichten $B$ mittels der Gleichungen \eqref{eq:fwhmMittel}, \eqref{eq:fwhmMittelErr} und \eqref{eq:b_fwhm} ermittelt werden. Die Ergebnisse sind Tabelle \ref{table:Erdmagnetfeld Ergebnisse} zu entnehmen.

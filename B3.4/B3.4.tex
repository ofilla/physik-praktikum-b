...
\hypersetup{
  pdftitle={B3.4: Positronen--Emissions--Tomographie},
  pdfcreator={\LaTeX}
}
...
Für die Medizin ist PET ein hervorragendes Werkzeug, besonders in der Onkologie. Das Verfahren kann Veränderung im Stoffwechsel erkennen, ehe sie ein anderes Verfahren nachweisen kann. So können Krebstumore früher erkannt und behandelt werden.
...
Radioaktiver Zerfall geschieht, wenn der Tochterkern eine höhere Bindungsenergie als der Mutterkern erhält. Es werden Neutronen benötigt, um die Coulombabstoßung \eqref{Coulombterm} der Protonen zu überwinden, jedoch verursachen zu viele Neutronen einen höheren Symmetrieterm in der Weizsäcker Massenformel\footnote{siehe Abschnitt \ref{weizsuxe4cker-massenformel}}. Daran kann man erklären, dass $\beta^-$--Zerfall dann geschehen kann, wenn der Kern zu viele Neutronen im Vergleich zu der Protonenanzahl hat.
...
\noindent
In diesem Versuch wird als Quelle $\ce{^{22}Na}$ verwendet, das eine Halbwertzeit $T_{1/2}\approx \pu{2.6 a}$ hat \cite{NuclideChart}. Dieses zerfällt über den $\beta^+$--Zerfall in einen angeregten Zustand des stabilen Isotops $\ce{^{22}Ne}$, welches unter Aussenden eines $\gamma$--Quants mit $E_\gamma = \pu{1275\, keV}$ in den Grundzustand zerfällt, siehe Abbildung \ref{abb:Skizze 22Na}. Beide Zerfälle geschehen koinzident.
...
Der Begriff der Paarvernichtung beschreibt die Zerstrahlung von einem Teilchen und seinem Antiteilchen, bei der die Teilchen in $\gamma$--Strahlung umgewandelt werden. Die Energie der Strahlung ist die Summe der kinetischen Energie dieser Teilchen sowie ihrer Ruhemassen. Die \nameref{paarbildung} ist der dazu inverse Effekt. \cite{Paarvernichtung}
...
\noindent
Der Wirkungsquerschnitt $\sigma_\mathrm{ph}$ wird durch die Ordnungszahl $Z$, die reduzierte Photonenenergie $\epsilon$ $\eqref{redEnergie}$, die Sommerfeld'sche Feinstrukturkonstante $\alpha$ und den Thomson--Wirkungsquerschnitt $\sigma_\mathrm{Th}^e$ bestimmt. \cite{Bethge}
...
\noindent
Hierbei sind weiterhin die Elektronenmasse $m_e$ und der Elektronenradius $r_e$ relevant.
...
Bei der Paarbildung entsteht aus einem Photon mit einer hohen Energie ein Teilchen--Antiteilchen--Paar, dieser Effekt dominiert bei Energien von etwa $\pu{5\, MeV}$. Die Umkehrung dieses Effektes nennt man \nameref{paarvernichtung}.
...
Der Paarungsterm wird betragsmäßig kleiner, je größer die Nukleonenzahl $A$ ist. Dies wird durch die folgende Gleichung beschrieben. Beide Nukleonensorten liefern betragsmäßig den gleichen Beitrag zu $E_P$. Bei $\mathrm{gg}$-- und $\mathrm{uu}$--Kernen addieren sich diese Werte zu einer nicht--verschwindenden Energie. Bei $\mathrm{gu}$-- und $\mathrm{ug}$--Kernen heben sich die Terme dagegen auf, weswegen der Paarungsterm hier verschwindet.
...
In diesem Versuch werden verschiedene elektronische Bauteile verwendet, die im Folgenden näher erläutert werden. In Abbildung \ref{abb:Schaltplan} ist eine Schaltskizze des Versuchs zu finden.
...
Es hat eine Breite von ca. $625 \mathrm{\, ns}$ und beim Peak eine Höhe von ca. $300 \mathrm{\, mV}$. Damit ist das Signal ungefähr so breit wie das aus dem TFA, die zeitliche Auflösung wurde durch das CFD  daher kaum verändert. Die Höhe ist wieder deutlich kleiner. Dies ist irrelevant für die Ausgabe des CFDs.
...
Uns wurde mitgeteilt, es gebe eine dritte, schwächere Probe. Weder anhand der Daten noch anhand der Grafiken können wir diese Quelle lokalisieren, weil es kaum signifikante Abhebungen erkennbar sind. Vermutlich ist diese sehr viel schwächer als die zwei großen Quellen.
...
Dies entspricht den Erwartungen. Damit zwei $\gamma$--Quanten mit Detektoren, deren Winkel kleiner als $180\,^\circ$ zueinander ist, koinzident gemessen werden, musste das Positron oder das Elektron bei der Paarvernichtung noch einen Restimpuls gehabt haben. Dies ist jedoch aufgrund der hohen Energieabhängigkeit des Wirkungsquerschnittes bei der Paarvernichtung deutlich unwahrscheinlicher als eine Paarvernichtung bei ruhendem Positron und Elektron.\footnote{Vergleiche Abschnitt \ref{paarvernichtung}} Daher wurden nur wenige Ergebnisse bei einem Messwinkel $\theta\neq 0\,^\circ$ erwartet.
...
Zuletzt wird die Winkelabhängigkeit aus der Detektion eines $\gamma$--Quants der Paarvernichtung und eines koinzident erzeugten $\gamma$--Quants aus dem sekundären Zerfall von $\ce{^{22}Ne}$ in den Grundzustand untersucht. Da dies zwei koinzidente, aber unabhängige Ereignisse sind, wird keine Winkelabhängigkeit erwartet.
...
Größenordnungstechnisch lässt sich dies unter der Annahme abschätzen, dass der Detektor einen deutlich kleineren Radius besitzt, als sein Abstand zur Probe beträgt. Sei der Radius des Detektors beispielsweise $r=1 \mathrm{\, cm}$ und sein Abstand zur Probe $R=10 \mathrm{\, cm}$.

...
\hypersetup{
  pdftitle={B3.1: Statistik der Kernphysik},
  pdfcreator={\LaTeX}
}
...
\noindent
Die Teilzeitabschnitte müssen infinitesimal klein sein, um ein exaktes Ergebnis zu liefern. Damit wird $k$ unendlich groß. Dadurch kann die Wahrscheinlichkeit $(1-\omega)$ für den Erhalt des Kerns durch eine Exponentialfunktion beschrieben werden. Entsprechend kann die Wahrscheinlichkeit $\omega$ für einen Zerfall innerhalb der Zeitspanne $t$ beschrieben werden.
...
\noindent
Das Ereignis $\omega$ des Zufallsexperiments kann direkt eine Zahl sein, wie beispielsweise die Augenzahl eines Würfelwurfs. Alternativ kann jedem Ereignis des Zufallsexperiments eine Zahl zugeordnet werden, beispielsweise können bei einem Münzwurf dem Ereignis $\mathrm{Kopf}$ der Wert $0$ und dem Ereignis $\mathrm{Zahl}$ eine $1$ zugeordnet werden.

Bei einem mehrfach wiederholten Zufallsexperiment liefert die Anzahl mit der ein bestimmtes Ereignis eintritt eine reelle Zahl.

\subsubsection{Wahrscheinlichkeitsdichte}
\label{Wahrscheinlichkeitsdichte}
Eine \textit{Wahrscheinlichkeitsdichte} (PDF\footnote{\emph{Probability Density Function}}) $f(x)$ bestimmt die Wahrscheinlichkeit $\mathbb{P}$, dass eine kontinuierliche Zufallsvariable $X$ einen Wert innerhalb eines Intervalls $[a,b]$ annimmt.

\begin{eqnarray}
	\int_{a}^{b} f(x) \dd{x} &=& \mathbb{P}(a \leq X \leq b)
		\label{eq:wahrscheinlichkeitsdichte}
\end{eqnarray}

\noindent
Die Wahrscheinlichkeitsdichte kann auch Werte über $1$ annehmen. Beispielsweise kann der Ort eines Punktteilchens durch eine $\delta$--Funktion beschrieben werden.

\subsubsection{Wahrscheinlichkeitsfunktion}
\label{Wahrscheinlichkeitsmassenfunktion}
Die Wahrscheinlichkeitsfunktion (PMF\footnote{\emph{Probability Mass Function}}) $f(x)$ gibt die Wahrscheinlichkeit an, dass eine diskrete Zufallsvariable $X$ genau den Wert $x$ annimmt.
...
\label{Wahrscheinlichkeitsverteilungsfunktion}
Eine \emph{kumulative Wahrscheinlichkeitsverteilungsfunktion} (CDF\footnote{\emph{Cumulative Distribution Function}}) oder Wahrscheinlichkeitsverteilung einer Zufallsvariable $X$ ist dagegen eine Funktion $F(x)$, die angibt, mit welcher Wahrscheinlichkeit $\mathbb{P}$ die Zufallsvariable $X$ einen Wert kleiner oder gleich $x$ annimmt.
...
\noindent
Für kontinuierliche Zufallsvariablen ist die CDF $F(x)$ die integrierte PDF $f(x)$, für diskrete Zufallsvariablen die Summe der PMF $f(x_i)$.

\begin{eqnarray}
	F(x) &=& \int_{- \infty}^{x} f(t) \dd{t} \\
	F(x) &=& \sum_{x_i \leq x} f(x_i)
		\label{eq:diskreteCDF}
\end{eqnarray}
...
\noindent
Die Poissonverteilung $P_\lambda(n)$ folgt als Grenzfall aus der Binomialverteilung $P(N, n, p)$ mit infinitesimal kleinen Schrittgrößen $(p\rightarrow 0)$ und unendlich vielen Schritten $(N\rightarrow \infty)$. Dabei bildet $\lambda\equiv Np$ eine Konstante. Die Poissonverteilung ist als Näherung für eine Binomialverteilung mit dem Mittelwert $m_B$ zu verwenden, falls folgende Bedingung erfüllt ist.

\begin{eqnarray}
	P_\lambda(n)  &=& \lim_{N \rightarrow \infty \atop p\rightarrow 0} P(N, n, p) \\
	m_B &\leq& 10
\end{eqnarray}
...
\noindent
Der Grenzwert der Poissonverteilung $P_\lambda(n)$ liefert für $\lambda \rightarrow \infty$ eine Gaußverteilung.

\begin{eqnarray}
	\lim_{\lambda \rightarrow \infty} P_\lambda(n) &=& \frac{1}{\sqrt{2 \pi \lambda}} e^{- \frac{(x-\lambda)^2}{2\lambda}}
\end{eqnarray}

\noindent
Weiterhin konvergiert die Binomialverteilung $P(N,n,p)$ nach dem \emph{Satz von Moivre--Laplace} für $N \rightarrow \infty$ mit der Bedingung $0 < p < 1$ gegen die Normalverteilung. Eine Faustregel besagt, dass die Normalverteilung schon eine gute Näherung für die Binomialverteilung liefert, sobald ihre Varianz $\sigma^2_B$ mindestens $9$ beträgt.

\begin{eqnarray}
	\lim_{N \rightarrow \infty} P(N,n,p)
		&=& \frac{1}{\sqrt{2 \pi \sigma_B^2}} e^{- \frac{(x - m_B)^2}{2\sigma_B^2}} \\
	\sigma^2_B &\geq& 9
\end{eqnarray}
...
\begin{align}
    F(x, 2f) &\eqspaced
        \int_0^x
            ...
            \,\mathrm dy \\
    \expval{\chi_f^2} &\eqspaced
        \int_0^\infty x\cdot f(x, f)
            \,\mathrm dx
        &&= f \\
    \sigma_{\chi^2} &\eqspaced
...
\noindent
Falls der Erwartungswert $m$ konstant ist, kann man weitere Schlussfolgerungen ziehen. Durch Einsetzen von $k_i$, $M$ sowie Gleichung $\eqref{ChiSquared}$ kann man $\chi_\mathrm{korr}^2$ \eqref{ChiKorrDef} vereinfachen und man erhält die folgende vereinfachte Relation. Hier sieht man, dass die korrigierte Abweichung $\chi^2_\mathrm{korr}$ größer als die nicht--korrigierte Abweichung $\chi^2$ ist, was kontraintuitiv wahrgenommen werden kann.
...
\noindent
Die Lösung dieses Gleichungssystems ergibt die Totzeit. Mathematisch erhält man immer zwei Lösungen, allerdings ist nur eine davon physikalisch relevant.
...
Die deutlichere Abweichung bei der gemeinsamen Messung  der Proben $A$ und $B$ kann ebenso wie bei der Poissonverteilung dadurch erklärt werden, dass mehr Zerfälle pro Sekunde stattfinden und deshalb auch mehr Zerfälle durch die Totzeit nicht detektiert werden.

Insgesamt sind damit die Beobachtungen sehr ähnlich zu denen der Poissonverteilung.
...
Für alle drei Verteilungsarten konnten die Messwerte Ergebnisse liefern, die mit jeweiligen theoretischen Verteilungen recht gut übereinstimmen. Unterschiede konnten erfasst und erklärt werden. Besonders gut ersichtlich ist der Einfluss der Totzeit.
